        ...
        <div class="card card-outline card-primary">
            <div class="card-body d-flex align-items-center justify-content-between flex-wrap py-3">
                <div class="d-flex align-items-center">
                    <div class="mr-3" style="width: 50px; height: 50px; border-radius: 12px; background: linear-gradient(135deg, #667eea, #764ba2); display: flex; align-items: center; justify-content: center;">
                        <i class="fas fa-key text-white fa-lg"></i>
                    </div>
                    <div>
                        <small class="text-muted d-block">Kode Absensi Anda</small>
                        <h4 class="mb-0 font-weight-bold" style="letter-spacing: 4px; color: #333;">
                            {{ $employee->attendance_code ?? 'Belum ada' }}
                        </h4>
                    </div>
                </div>
                <small class="text-muted"><i class="fas fa-info-circle mr-1"></i>Gunakan kode ini saat absen di halaman scan</small>
            </div>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-lg col-6">
        <div class="small-box bg-info border-0 shadow-sm">
            <div class="inner">
    ...
    <div class="col-lg col-6">
        <div class="small-box bg-warning border-0 shadow-sm text-dark">
            <div class="inner">
                <h3>{{ $totalLate ?? 0 }}</h3>
                <p>Total Terlambat</p>
            </div>
            <div class="icon">
                <i class="fas fa-clipboard-list text-dark-50"></i>
            </div>
             <a href="{{ route('attendance.history', ['status' => 'Late']) }}" class="small-box-footer text-dark">Lihat Detail <i class="fas fa-arrow-circle-right"></i></a>
        </div>
    </div>
    <div class="col-lg col-6">
        <div class="small-box bg-teal border-0 shadow-sm text-white">
            <div class="inner">
    ...
    <div class="col-lg col-6">
        <div class="small-box bg-danger border-0 shadow-sm">
            <div class="inner">
                <h3>{{ $totalAlpha ?? 0 }}</h3>
                <p>Total Alpha</p>
            </div>
    ...
    <div class="col-lg col-12">
        <div class="small-box bg-indigo border-0 shadow-sm text-white" style="background-color: #6610f2 !important;">
            <div class="inner">
                <h3>{{ Auth::user()->sisa_cuti }}</h3>
                <p>Sisa Jatah Cuti</p>
            </div>
            <div class="icon">
                <i class="fas fa-calendar-minus opacity-50"></i>
            </div>
             <a href="{{ route('leave.index') }}" class="small-box-footer">Status Cuti <i class="fas fa-arrow-circle-right"></i></a>
        </div>
    </div>
</div>
        ...
        <div class="card card-primary card-outline">
            ...
            <div class="card-body box-profile d-flex flex-column justify-content-center text-center p-4">
                @if(isset($activeLeave) && $activeLeave)
                    <div class="icon-circle mb-3 mx-auto" style="width: 100px; height: 100px; border-radius: 50%; display: flex; align-items: center; justify-content: center; background-color: #6366f1;">
                        <i class="fas fa-calendar-check fa-4x text-white"></i>
                    </div>
                    <h3 class="profile-username">Sedang Cuti</h3>
                    <p class="text-muted mb-4">{{ \Carbon\Carbon::today()->translatedFormat('l, d F Y') }}</p>
                    <div class="alert alert-info border-0 shadow-none text-left" style="background: #eef2ff; color: #4338ca;">
                        <i class="fas fa-info-circle mr-2"></i> Anda sedang cuti sampai tanggal {{ \Carbon\Carbon::parse($activeLeave->end_date)->translatedFormat('d F Y') }}.
                    </div>
                    <a href="{{ route('leave.index') }}" class="btn btn-outline-primary btn-block">
                        <i class="fas fa-calendar-alt mr-2"></i> Lihat Detail Cuti
                    </a>
                ...
                @else
                    ...
                    <a href="{{ route('attendance.permission.create') }}" class="btn btn-outline-warning btn-block mb-3">
                        <i class="fas fa-file-alt mr-2"></i> Ajukan Izin / Cuti
                    </a>

                    <a href="{{ route('leave.index') }}" class="btn btn-outline-primary btn-block">
                        <i class="fas fa-history mr-2"></i> Riwayat & Status Cuti
                    </a>
                @endif
            </div>
        </div>
